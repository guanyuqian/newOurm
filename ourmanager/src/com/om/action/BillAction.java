...
public class BillAction extends ActionSupport {
	...
	public String createBill() throws Exception {
		return SUCCESS;
		/*
		 * User user; Om om; ServletRequest request =
		 * ServletActionContext.getRequest(); HttpSession session =
		 * ((HttpServletRequest) request).getSession(); user = (User)
		 * session.getAttribute("user"); om = (Om) session.getAttribute("om");
		 * Bill newbill = new Bill(bill);
		 * 
		 * newbill.setBillid(100011); newbill.setUserByCreaterUserid(user);
		 * newbill.setUserByCheckerUserid(user); newbill.setBillStatus(1);
		 * newbill.setOm(om); newbill.setDeleteflag(false); if
		 * (billservice.createBill(newbill, users)) { bill = newbill; return
	...
	public String preAddUserBill() {
		AllUser = billservice.loadAddUserBill();
